...
def generate_service(
    ...
    if style == "kubernetes" and crd_kinds:
        return _generate_crd_service(name, config, output_dir, specs_dir)
...
def _generate_crd_service(
    ...
    try:
        ...
        # Choose loading strategy: ArtifactHub chart vs local spec.
        if config.get("chart"):
            from idi.generation.crd.schema_loader import load_crd_schemas
            schemas, values = load_crd_schemas(name, config)
        elif specs_dir:
            schemas = _load_schemas_from_spec(name, config, specs_dir)
        else:
            schemas = []
